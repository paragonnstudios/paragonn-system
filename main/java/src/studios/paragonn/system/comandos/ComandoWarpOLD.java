...
public class ComandoWarpOLD implements CommandExecutor {
	...
	public boolean onCommand(CommandSender s, Command cmd, String lbl, String[] args) {
		...
		if (args.length == 2) {
			...
			if (w.enviarMensagemPlayerTeleportado() && w.getMensagemPlayerTeleportado() != null && !w.getMensagemPlayerTeleportado().isEmpty()) {
				target.sendMessage(w.getMensagemPlayerTeleportado().replace("%player%", s.getName()));
			}
			if (w.getMensagemPlayerTeleportadoStaff() != null && !w.getMensagemPlayerTeleportadoStaff().isEmpty()) {
