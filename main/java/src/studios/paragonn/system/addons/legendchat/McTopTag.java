...
public class McTopTag implements Listener {
    ...
    public void aoEnviarMenssagem(ChatMessageEvent e) {
        if (
            e.getSender().getName().equals(playerTopOne) &&
            e.getTags().contains("mctop")
        ) {
    ...
    public static void checkMCTop() {
        TTask = new BukkitRunnable() {
            @Override
            public void run() {
                List<PlayerStat> tops =
                    DatabaseManagerFactory.getDatabaseManager().readLeaderboard(
                        null,
                        1,
                        1
                    );
            ...
            Main.get(),
            60L,
            Settings.mcTopTag_Tempo_De_Checagem * 20L
        );
    }
}
